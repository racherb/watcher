...
test:test('single_file_deletion:file_not_exist', function(test)
    ...
    local ans = kit.deletion({folder_not_exist_yet})
...
test:test('multiple_file_deletion:list_experiments', function(test)
    ...
    local ans, mssg = kit.deletion(file_list, MAXWAIT, INTERVAL)
    ...
    local ans, mssg = kit.deletion(file_list_2, MAXWAIT, INTERVAL)
    ...
    local ans, mssg = kit.deletion(file_list_3, MAXWAIT, INTERVAL, options)
    test:is(ans, true, TEST.id12)
    test:is(mssg, 'FW_MATCH_DELETED', TEST.id2)

    for _=1,10 do os.tmpname() end
    local file_pattern = {'/tmp/*'}
    local ans, mssg = kit.deletion(file_pattern, MAXWAIT, INTERVAL)
    ...
    fiber.create(remove_tmp_files, 3)
    local ans, mssg = kit.deletion(folder, MAXWAIT, INTERVAL)
    ...
    local ans, mssg = kit.deletion(watcher_mix, MAXWAIT, INTERVAL)
    ...
    fiber.create(remove_file, '/tmp/f_b', 1)
    fiber.create(remove_file, '/tmp/f_c', 1)
    local options = {'MTIME_DSC', 3, 2}
    local ans, mssg = kit.deletion({'/tmp/f_*'}, MAXWAIT, INTERVAL, options)
    print(ans)
    print(mssg)
